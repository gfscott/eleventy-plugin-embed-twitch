...
module.exports = function(media, options) {
   ...
   */
	// rome-ignore lint/js/noExtraBooleanCast
	if (!!!options.parent) {
		console.error(
			`⚠️  ${chalk.red("Error embedding Twitch video")}
  Eleventy plugin ${chalk.cyan("eleventy-plugin-embed-twitch")} requires a \`parent\` option to be set.
  🔗  See ${chalk.cyan.underline(
				"https://bit.ly/11ty-plugin-twitch-parent-error",
			)} for details.`,
		);
	}
	...
	let out = `<div id="twitch-${media.id}" class="${options.embedClass}" `;
